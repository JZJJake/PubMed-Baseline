...
async def process_single_url(task_id: str, current_url: str, start_url: str, base_path: str, browser) -> None:
    ...
    try:
        ...
        # Listen for JSON API responses for SPAs
        api_responses = []
        async def handle_response(response):
            try:
                # Capture anything that looks like JSON data
                if "json" in response.headers.get("content-type", "").lower() or "json" in response.url.lower():
                    data = await response.json()
                    # Only keep somewhat substantial payloads to avoid clutter
        ...
        # Save intercepted API data and try to extract URLs
        if api_responses:
            api_data_path = os.path.join(page_path, "api_data.json")
            ...
            api_md_content = "\n\n## Intercepted API Data (SPA Content)\n\n"
            api_links = set()

            # Recursive function to find urls or build text from JSON
            def parse_json_node(node, depth=0):
                text = ""
                if isinstance(node, dict):
                    for k, v in node.items():
                        if isinstance(v, (dict, list)):
                            text += "  " * depth + f"- **{k}**:\n"
                            text += parse_json_node(v, depth + 1)
                        else:
                            text += "  " * depth + f"- **{k}**: {v}\n"
                            # Basic heuristic to find URLs hidden in JSON
            ...
            for response in api_responses:
                api_md_content += f"### Source: `{response['url']}`\n"
                api_md_content += parse_json_node(response['data'])
                api_md_content += "\n---\n"

            # Append to main markdown content
            markdown_content += api_md_content

            # Process discovered links from API data
            valid_api_links = []
            ...
            if valid_api_links:
                await asyncio.to_thread(db_manager.add_discovered_urls, task_id, current_url, list(set(valid_api_links)))
